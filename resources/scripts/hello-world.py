...
def generate_pesh_password(USERNAME, USERPASS, REQUEST_TYPE, HMCS, START_DATE, END_DATE):
    ...
    for i in HMCS.split(','):
        ...
        API_INPUT_DATA['start']         = str(START_DATE)
        API_INPUT_DATA['stop']          = str(END_DATE)
...
def get_the_month_start_and_end_dates():
    """
    This function is to find out the month start and end dates
    """
    ...
    input_dt = datetime(START_DATE.year, START_DATE.month, 1)
...
def main(argv):
    ...
    opts, args = getopt.getopt(argv, "hu:p:r:sef", ["hmc="])
    ...
    if START_DATE == "" or END_DATE == "":
        get_the_month_start_and_end_dates()
